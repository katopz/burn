//! Integration tests for SFT training pipeline with LoRA-adapted Gemma 4.
//!
//! Uses a tiny model configuration (small vocab, 2 layers, 4 heads) to validate
//! the training pipeline end-to-end without requiring real weights.
//!
//! Tests mirror [`lora_gemma2`] tests but adapted for Gemma 4 architecture:
//! - No `mask_pad` in SFTTrainingBatch (Gemma 4 batch is simpler)
//! - Uses [`apply_lora_to_gemma4`] with Gemma 4 specific LoRA wrappers
//! - Includes Q/K norms, KV sharing, PLE, layer scalars (all Gemma 4 features)
...
use lora_gemma4::model::Gemma4Model;
use lora_gemma4::model_lora::SFTTrainingBatch;
use lora_gemma4::model_lora::{
    Gemma4ForSFT, Gemma4ModelLora, apply_lora_to_gemma4, count_lora_params, count_total_params,
};
use lora_gemma4::types::{Gemma4Config, LoraTarget};
...
fn make_lora_model() -> Gemma4ModelLora<TestAD> {
    let device = device();
    let config = tiny_config();
    let model = Gemma4Model::<TestAD>::new(&config, &device);
...
///
/// Note: Gemma 4 SFTTrainingBatch has no `mask_pad` field (unlike Gemma 2).
fn make_batch(
    batch_size: usize,
    seq_len: usize,
    device: &NdArrayDevice,
) -> SFTTrainingBatch<TestAD> {
...
fn test_train_step_produces_loss_and_grads() {
    ...
    let output = <Gemma4ForSFT<TestAD> as TrainStep>::step(&sft_model, make_batch(2, 8, &device));
...
fn test_inference_step_produces_loss() {
    ...
    let output = <Gemma4ForSFT<TestAD> as InferenceStep>::step(&sft_model, batch);
...
fn test_training_reduces_loss() {
    ...
    // Initial loss
    let initial_output =
        <Gemma4ForSFT<TestAD> as InferenceStep>::step(&sft_model, make_batch(2, 8, &device));
    ...
    for step in 0..20 {
        let batch = make_batch(2, 8, &device);
        let train_output = <Gemma4ForSFT<TestAD> as TrainStep>::step(&model, batch);
        model = model.optimize(&mut optimizer, lr, train_output.grads);

        // Evaluate after step
        let eval_output =
            <Gemma4ForSFT<TestAD> as InferenceStep>::step(&model, make_batch(2, 8, &device));
...
fn test_merge_after_training() {
    ...
    for _ in 0..5 {
        let batch = make_batch(2, 8, &device);
        let output = <Gemma4ForSFT<TestAD> as TrainStep>::step(&model, batch);
...
fn test_adapter_save_load_after_training() {
    ...
    let lora_config = LoraConfig::new(4).with_alpha(8.0).with_bias(LoraBias::None);
    let model = Gemma4Model::<TestAD>::new(&config, &device);
    let lora_model = apply_lora_to_gemma4(model, &lora_config, LoraTarget::all_targets(), &device);
    let sft_model = Gemma4ForSFT::new(lora_model, 0);
    ...
    let fresh_model = Gemma4Model::<TestAD>::new(&config, &device);
    let fresh_lora = apply_lora_to_gemma4(
        fresh_model,
        &lora_config,
...
fn test_param_counts_all_targets() {
    ...
    // q_proj: A[32,4]+B[4,32]=256
    // k_proj: A[32,4]+B[4,16]=192 (head_dim*num_kv_heads = 8*2 = 16)
    // v_proj: A[32,4]+B[4,16]=192
    // o_proj: A[32,4]+B[4,32]=256
    // gate_proj: A[32,4]+B[4,64]=384
    // up_proj: A[32,4]+B[4,64]=384
    // down_proj: A[64,4]+B[4,32]=384
...
fn test_param_counts_attention_only() {
    ...
    let lora_model = apply_lora_to_gemma4(
        model,
        &lora_config,
    ...
    // NOTE: apply_lora_to_gemma4 wraps ALL projections with LoRA but freezes non-targets.
...
fn test_gradient_flow_only_lora() {
    ...
    let batch = make_batch(2, 8, &device);
    let output = <Gemma4ForSFT<TestAD> as TrainStep>::step(&sft_model, batch);
...
// ---------------------------------------------------------------------------
// Tests: End-to-end Manual Training Loop
// ---------------------------------------------------------------------------
...
fn test_supervised_training_runs() {
    ...
    // Manual training loop (no batcher/tokenizer needed for synthetic data)
    let mut optimizer = AdamConfig::new().init();
    let lr = 1e-3;
