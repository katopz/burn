...
///
/// HuggingFace Gemma 4 weights are stored in BF16, but Metal/WGPU
...
pub fn load_gemma4_weights<B: Backend>(
    ...
    load_gemma4_weights_dtype(model, path, device, DType::F32)
}

/// Load Gemma 4 weights with explicit target dtype.
///
...
pub fn load_gemma4_weights_dtype<B: Backend>(
    model: &mut Gemma4Model<B>,
    path: &Path,
    device: &B::Device,
    target_dtype: DType,
