...
pub struct LoraLinear<B: Backend> {
    ...
    /// Skipped from module persistence via `#[module(skip)]`.
...
impl<B: Backend> LoraLinear<B> {
    ...
    pub fn into_adapter(self) -> LoraLinearAdapter<B> {
        LoraLinearAdapter::new(self.lora_a, self.lora_b, self.config)
    }
...
mod tests {
    ...
    ///
    /// Expected:
    /// - base(x) = x @ W + bias = [1,1,1] @ [[1,2],[3,4],[5,6]] + [0.1,0.2] = [9.1, 12.2]
    /// - lora(x) = (x @ A @ B) * scaling = [0.6] @ [[0.5, 0.6]] * 2.0 = [0.6, 0.72]
