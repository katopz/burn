...
//! ```sh
//! # CPU backend (testing)
//! cargo run -p lora-gemma4 --features ndarray --bin sft-train -- \
//!   --dataset input/train.jsonl \
//!   --output-dir /tmp/sft-output
//!
//! # WGPU/Metal backend (macOS)
//! cargo run -p lora-gemma4 --features wgpu --bin sft-train -- \
//!   --dataset input/train.jsonl \
//!   --output-dir /tmp/sft-output \
//!   --weights /path/to/model.safetensors
//!
//! # Metal native backend (macOS)
//! cargo run -p lora-gemma4 --features metal --bin sft-train -- \
//!   --dataset input/train.jsonl \
//!   --output-dir /tmp/sft-output \
//!   --weights ~/.cache/huggingface/hub/models--unsloth--gemma-4-E4B-it/snapshots/*/model.safetensors
//! ```
//!
//! # Arguments
//!
...
//! | `--model-name` | `unsloth/gemma-4-E4B-it` | HF model name for tokenizer |
...
//! | `--seed` | `42` | Random seed |
//! | `--tokenizer` | (none) | Local tokenizer.json path (overrides --model-name) |
...
impl SftArgs {
    ...
    fn parse() -> Self {
        let mut args = Self {
            ...
            model_name: "unsloth/gemma-4-E4B-it".into(),
            weights: None,
            lora_rank: 16,
            lora_alpha: 32.0,
            epochs: 3,
...
fn print_usage() {
    ...
    eprintln!(
        "  --model-name <NAME>      HF model name for tokenizer (default: unsloth/gemma-4-E4B-it)"
    );
...
fn run<B: AutodiffBackend>(args: SftArgs, device: B::Device) {
    log::info!("=== LoRA SFT Training for Gemma 4 ===\n");
    ...
    // 5. Build Model
    // -----------------------------------------------------------------------
    log::info!("[4/8] Building Gemma 4 model");
    ...
    log::info!(
        "  Sliding window: {}, pattern: 5:1 (5 sliding + 1 full)",
        config.sliding_window
    );
    log::info!(
        "  KV shared layers: {}, PLE dim: {}",
        config.num_kv_shared_layers,
        config.hidden_size_per_layer_input
    );

    // Create on inner backend (no autodiff) so loaded weights become leaf tensors
    let mut inner_model = Gemma4Model::<B::InnerBackend>::new(&config, &device);
    ...
    match &args.weights {
        Some(weights_path) => {
            ...
            match load_gemma4_weights_dtype(
                &mut inner_model,
    ...
    let model = Gemma4Model::<B>::from_inner(inner_model);
    ...
    let sft_model = Gemma4ForSFT::new(lora_model, pad_token_id);
    ...
    // -----------------------------------------------------------------------
    log::info!("\n=== Summary ===");
    log::info!("  Model: Gemma 4 E4B (42 layers, 2560 hidden)");
